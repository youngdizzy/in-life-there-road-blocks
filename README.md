...
DISCOVER → RAISE → INFLUENCE → GROW → first EVOLVE → COLLECT (two
Critters), and the monetization systems in the table above layer on top of
it without changing that loop. Multi-event scheduling, premium habitat
re-skins, and evolution reroll remain schema-only or not started.
